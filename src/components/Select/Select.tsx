    ...
    if (search) {
      ...
      if (key.length === 1) {
        ...
        this.setState(
          prevState => ({
            searchValue:
              prevState.searchValue.slice(0, selectionStart) + key + prevState.searchValue.slice(selectionStart)
          }),
          () => {
            this.focusSearch();
            this.__searchRef.current!.setSelectionRange(selectionStart + 1, selectionStart + 1);
          }
        );
